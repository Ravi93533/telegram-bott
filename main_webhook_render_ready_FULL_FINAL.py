...
async def kanal_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if member.status in ("member", "administrator", "creator"):
            chat_obj = await context.bot.get_chat(q.message.chat.id)
            base_perm = getattr(chat_obj, "permissions", None)
            if base_perm is None:
                base_perm = ChatPermissions(can_send_messages=True, can_invite_users=True)
...
async def majbur_filter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        base_perm = getattr(chat_obj, "permissions", None)
        # Fallback: allow inviting if chat didn't return defaults
        if base_perm is None:
            base_perm = ChatPermissions(can_invite_users=True)

        # Copy all default permissions, only disable sending messages
